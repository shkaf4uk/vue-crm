...
export default {
  ...
  async mounted() {
    ...
    this.categories = categories.map((cat) => {
      ...
      let excessBy
      let left
      if (this.$store.getters.info.locate === 'ru-RU') {
        excessBy = 'Превышение на:'
        left = 'Осталось:'
      } else if (this.$store.getters.info.locate === 'en-US') {
        excessBy = 'Excess by:'
        left = 'Left:'
      }

      const tooltip = `${tooltipValue < 0 ? excessBy : left} ${currencyFilter(
